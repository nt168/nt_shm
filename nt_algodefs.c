...
{								\
	...
	while (len >= 12)
	{
		a = a + (p[0] + ((nt_hash_t)p[1] << 8) + ((nt_hash_t)p[2]  << 16) + ((nt_hash_t)p[3]  << 24));
		b = b + (p[4] + ((nt_hash_t)p[5] << 8) + ((nt_hash_t)p[6]  << 16) + ((nt_hash_t)p[7]  << 24));
		c = c + (p[8] + ((nt_hash_t)p[9] << 8) + ((nt_hash_t)p[10] << 16) + ((nt_hash_t)p[11] << 24));
	...
	switch (len)
	{
		case 11:	c = c + ((nt_hash_t)p[10] << 24);
		case 10:	c = c + ((nt_hash_t)p[9] << 16);
		case 9:		c = c + ((nt_hash_t)p[8] << 8);
		case 8:		b = b + ((nt_hash_t)p[7] << 24);
		case 7:		b = b + ((nt_hash_t)p[6] << 16);
		case 6:		b = b + ((nt_hash_t)p[5] << 8);
		case 5:		b = b + p[4];
		case 4:		a = a + ((nt_hash_t)p[3] << 24);
		case 3:		a = a + ((nt_hash_t)p[2] << 16);
		case 2:		a = a + ((nt_hash_t)p[1] << 8);
		case 1:		a = a + p[0];
	}
...
nt_hash_t	nt_hash_murmur2(const void *data, size_t len, nt_hash_t seed)
{
	...
	const nt_hash_t	m = 0x5bd1e995u;
	const nt_hash_t	r = 24;
...
{
	return NT_DEFAULT_PTR_HASH_ALGO(data, NT_PTR_SIZE, NT_DEFAULT_HASH_SEED);
}

nt_hash_t	nt_default_uint64_hash_func(const void *data)
{
	return NT_DEFAULT_UINT64_HASH_ALGO(data, sizeof(nt_uint64_t), NT_DEFAULT_HASH_SEED);
}

nt_hash_t	nt_default_string_hash_func(const void *data)
{
	return NT_DEFAULT_STRING_HASH_ALGO(data, strlen((const char *)data), NT_DEFAULT_HASH_SEED);
}

nt_hash_t	nt_default_uint64_pair_hash_func(const void *data)
{
	const nt_uint64_pair_t	*pair = (const nt_uint64_pair_t *)data;

	nt_hash_t		hash;

	hash = NT_DEFAULT_UINT64_HASH_FUNC(&pair->first);
	hash = NT_DEFAULT_UINT64_HASH_ALGO(&pair->second, sizeof(pair->second), hash);
...
{
	const nt_uint64_t	*i1 = (const nt_uint64_t *)d1;
	const nt_uint64_t	*i2 = (const nt_uint64_t *)d2;
...
{
	const nt_uint64_t	*p1 = *(const nt_uint64_t **)d1;
	const nt_uint64_t	*p2 = *(const nt_uint64_t **)d2;

	return nt_default_uint64_compare_func(p1, p2);
}
...
{
	const nt_uint64_pair_t	*p1 = (const nt_uint64_pair_t *)d1;
	const nt_uint64_pair_t	*p2 = (const nt_uint64_pair_t *)d2;

	NT_RETURN_IF_NOT_EQUAL(p1->first, p2->first);
	NT_RETURN_IF_NOT_EQUAL(p1->second, p2->second);
