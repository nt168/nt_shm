 ...
 *                                                                            *
 *     when a chunk is free, the first 2 * NT_PTR_SIZE bytes of allocatable  *
...
static nt_uint64_t	mem_proper_alloc_size(nt_uint64_t size);
static int	mem_bucket_by_size(nt_uint64_t size);

static void	mem_set_chunk_size(void *chunk, nt_uint64_t size);
static void	mem_set_used_chunk_size(void *chunk, nt_uint64_t size);
...
static void	mem_link_chunk(nt_mem_info_t *info, void *chunk);
static void	mem_unlink_chunk(nt_mem_info_t *info, void *chunk);

static void	*__mem_malloc(nt_mem_info_t *info, nt_uint64_t size);
static void	*__mem_realloc(nt_mem_info_t *info, void *old, nt_uint64_t size);
static void	__mem_free(nt_mem_info_t *info, void *ptr);

#define MEM_SIZE_FIELD		sizeof(nt_uint64_t)

#define MEM_FLG_USED		((__UINT64_C(1))<<63)

#define FREE_CHUNK(ptr)		(((*(nt_uint64_t *)(ptr)) & MEM_FLG_USED) == 0)
#define CHUNK_SIZE(ptr)		((*(nt_uint64_t *)(ptr)) & ~MEM_FLG_USED)

#define MEM_MIN_SIZE		__UINT64_C(128)
#define MEM_MAX_SIZE		__UINT64_C(0x1000000000)	/* 64 GB */

#define MEM_MIN_ALLOC	24	/* should be a multiple of 8 and at least (2 * NT_PTR_SIZE) */
...
static void	mem_set_chunk_size(void *chunk, nt_uint64_t size)
{
	*(nt_uint64_t *)chunk = size;
	*(nt_uint64_t *)((char *)chunk + MEM_SIZE_FIELD + size) = size;
}

static void	mem_set_used_chunk_size(void *chunk, nt_uint64_t size)
{
	*(nt_uint64_t *)chunk = MEM_FLG_USED | size;
	*(nt_uint64_t *)((char *)chunk + MEM_SIZE_FIELD + size) = MEM_FLG_USED | size;
}
...
static void	*mem_get_next_chunk(void *chunk)
{
	return *(void **)((char *)chunk + MEM_SIZE_FIELD + NT_PTR_SIZE);
}

static void	mem_set_next_chunk(void *chunk, void *next)
{
	*(void **)((char *)chunk + MEM_SIZE_FIELD + NT_PTR_SIZE) = next;
}
...
static void	**mem_ptr_to_next_field(void *chunk, void **first_chunk)
{
	return (NULL != chunk ? (void **)((char *)chunk + MEM_SIZE_FIELD + NT_PTR_SIZE) : first_chunk);
}
...
static void	*__mem_malloc(nt_mem_info_t *info, nt_uint64_t size)
{
	...
	if (index == MEM_BUCKET_COUNT - 1)
	{
		...
		int		counter = 0;
		nt_uint64_t	skip_min = __UINT64_C(0xffffffffffffffff), skip_max = __UINT64_C(0);
		...
		if (0 == info->allow_oom)
		{
			if (NULL == chunk)
			{
				nt_log(LOG_LEVEL_CRIT, "__mem_malloc: skipped %d asked %lu skip_min %lu skip_max %lu",
						counter, size, skip_min, skip_max);
			}
			else if (counter >= 100)
			{
				nt_log(LOG_LEVEL_DEBUG, "__mem_malloc: skipped %d asked %lu skip_min %lu skip_max %lu"
						" size %lu", counter, size, skip_min, skip_max, CHUNK_SIZE(chunk));
			}
		}
	}
...
static void	*__mem_realloc(nt_mem_info_t *info, void *old, nt_uint64_t size)
{
	...
	nt_uint64_t	chunk_size, new_chunk_size;
	int		next_free;
...
{
	const char		*__function_name = "nt_mem_create";

	int			shm_id, index, ret = FAIL;
	void			*base;

	descr = NT_NULL2STR(descr);
	param = NT_NULL2STR(param);

	nt_log(LOG_LEVEL_DEBUG, "In %s() param:'%s' size:" NT_FS_SIZE_T, __function_name, param,
			(nt_fs_size_t)size);

	/* allocate shared memory */

	if (4 != NT_PTR_SIZE && 8 != NT_PTR_SIZE)
	{
		*error = nt_dsprintf(*error, "failed assumption about pointer size (" NT_FS_SIZE_T " not in {4, 8})",
				(nt_fs_size_t)NT_PTR_SIZE);
		goto out;
	}

	if (!(MEM_MIN_SIZE <= size && size <= MEM_MAX_SIZE))
	{
		*error = nt_dsprintf(*error, "requested size " NT_FS_SIZE_T " not within bounds [" NT_FS_UI64
				" <= size <= " NT_FS_UI64 "]", (nt_fs_size_t)size, MEM_MIN_SIZE, MEM_MAX_SIZE);
		goto out;
	}

	if (-1 == (shm_id = shmget(IPC_PRIVATE, size, 0600)))
	{
		*error = nt_dsprintf(*error, "cannot get private shared memory of size " NT_FS_SIZE_T " for %s: %s",
				(nt_fs_size_t)size, descr, nt_strerror(errno));
		goto out;
	}

	if ((void *)(-1) == (base = shmat(shm_id, NULL, 0)))
	{
		*error = nt_dsprintf(*error, "cannot attach shared memory for %s: %s", descr, nt_strerror(errno));
		goto out;
	}

	if (-1 == shmctl(shm_id, IPC_RMID, NULL))
		nt_error("cannot mark shared memory %d for destruction: %s", shm_id, nt_strerror(errno));

	ret = SUCCEED;

	/* allocate nt_mem_info_t structure, its buckets, and description inside shared memory */
	...
	*info = ALIGN8(base);
	...
	(*info)->total_size = (nt_uint64_t)((char *)((*info)->hi_bound) - (char *)((*info)->lo_bound) -
			2 * MEM_SIZE_FIELD);
	...
	nt_log(LOG_LEVEL_DEBUG, "valid user addresses: [%p, %p] total size: " NT_FS_SIZE_T,
			(char *)(*info)->lo_bound + MEM_SIZE_FIELD,
			(char *)(*info)->hi_bound - MEM_SIZE_FIELD,
			(nt_fs_size_t)(*info)->total_size);
out:
...
{
	const char	*__function_name = "nt_mem_malloc";

	void		*chunk;

	if (NULL != old)
	{
		nt_log(LOG_LEVEL_CRIT, "[file:%s,line:%d] %s(): allocating already allocated memory",
	...
	{
		nt_log(LOG_LEVEL_CRIT, "[file:%s,line:%d] %s(): asking for a bad number of bytes (" NT_FS_SIZE_T
				")", file, line, __function_name, (nt_fs_size_t)size);
		exit(EXIT_FAILURE);
	}
	...
	if (NULL == chunk)
	{
		...
		nt_log(LOG_LEVEL_CRIT, "[file:%s,line:%d] %s(): out of memory (requested " NT_FS_SIZE_T " bytes)",
				file, line, __function_name, (nt_fs_size_t)size);
		nt_log(LOG_LEVEL_CRIT, "[file:%s,line:%d] %s(): please increase %s configuration parameter",
...
{
	const char	*__function_name = "nt_mem_realloc";
...
{
	const char	*__function_name = "nt_mem_free";

	if (NULL == ptr)
	{
		nt_log(LOG_LEVEL_CRIT, "[file:%s,line:%d] %s(): freeing a NULL pointer",
				file, line, __function_name);
		exit(EXIT_FAILURE);
	}
...
{
	const char	*__function_name = "nt_mem_clear";
...
void	nt_mem_dump_stats(nt_mem_info_t *info)
{
	void		*chunk;
	int		index;
	nt_uint64_t	counter, total, total_free = 0;
	nt_uint64_t	min_size = __UINT64_C(0xffffffffffffffff), max_size = __UINT64_C(0);

	nt_log(LOG_LEVEL_DEBUG, "=== memory statistics for %s ===", info->mem_descr);
	...
	for (index = 0; index < MEM_BUCKET_COUNT; index++)
	{
		...
		{
			total_free += counter;
			nt_log(LOG_LEVEL_DEBUG, "free chunks of size %2s %3d bytes: %8ld",
					index == MEM_BUCKET_COUNT - 1 ? ">=" : "",
					MEM_MIN_BUCKET_SIZE + 8 * index, counter);
		}
	}

	nt_log(LOG_LEVEL_DEBUG, "min chunk size: %10lu bytes", min_size);
	nt_log(LOG_LEVEL_DEBUG, "max chunk size: %10lu bytes", max_size);

	total = (info->total_size - info->used_size - info->free_size) / (2 * MEM_SIZE_FIELD) + 1;
	nt_log(LOG_LEVEL_DEBUG, "memory of total size %lu bytes fragmented into %lu chunks", info->total_size, total);
	nt_log(LOG_LEVEL_DEBUG, "of those, %10lu bytes are in %8lu free chunks", info->free_size, total_free);
	nt_log(LOG_LEVEL_DEBUG, "of those, %10lu bytes are in %8lu used chunks", info->used_size, total - total_free);
...
{
	const char	*__function_name = "nt_mem_required_size";

	size_t		size = 0;

	nt_log(LOG_LEVEL_DEBUG, "In %s() size:" NT_FS_SIZE_T " chunks_num:%d descr:'%s' param:'%s'",
			__function_name, (nt_fs_size_t)size, chunks_num, descr, param);
	...
	size += 7;					/* ensure we allocate enough to 8-align nt_mem_info_t */
	size += sizeof(nt_mem_info_t);
	size += NT_PTR_SIZE - 1;			/* ensure we allocate enough to align bucket pointers */
	size += NT_PTR_SIZE * MEM_BUCKET_COUNT;
	size += strlen(descr) + 1;
	size += strlen(param) + 1;
	...
	nt_log(LOG_LEVEL_DEBUG, "End of %s() size:" NT_FS_SIZE_T, __function_name, (nt_fs_size_t)size);
